...
class Settings(BaseModel):
    ...
    min_year: Optional[int] = Field(
        ...
        ge=1000,
        le=3000,
        description="Minimum year for search range"
    )
    max_year: Optional[int] = Field(
        default=None,
        ge=1000,
        le=3000,
        description="Maximum year for search range"
    )
    ...
    sleep_timer: float = Field(
        default=1.0,
        ge=0.1,
        description="Sleep between requests (seconds)"
    )
    ...
    # Keywords
    keyword_column: str = Field(
        default="title",
        description="CSV column name to use as keyword"
    )
    ...
    proximity_distance: int = Field(
        ...
        ge=1,
        le=100,
        description="Proximity search distance (words)"
    )
